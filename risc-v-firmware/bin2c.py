...
def main(argv):
    ...
    outfile.write("#include \"" + output_file[:-2] + ".h"+"\"\n\n")
    outfile.write("__attribute__((section(\".memory\"))) const unsigned char " + array_name + "[" +  "N_" + array_name.upper() + "] = {")
    ...
    outfile.write("#define N_" + array_name.upper() +" "+ str(len(bin_file_contents))+"\n")

    outfile.write("\n")
    outfile.write("extern const unsigned char " + array_name + "[" + "N_" + array_name.upper() + "];\n")
    outfile.write("\n")
    outfile.write("#endif")
    outfile.close()
