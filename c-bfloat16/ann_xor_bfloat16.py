...
x1 = np.array([[0,0],[0,1],[1,0],[1,1]],dtype=bfloat16) # Inputs
y1 = np.array([[0], [1], [1], [0]],dtype=bfloat16) # Outputs
y1 = np.array([[0], [1], [1], [1]],dtype=bfloat16) # Outputs

# Defining the sigmoid activation function
def sigmoid(z):
    z = bfloat16(1)/(bfloat16(1 )+ np.exp(-z))
    return z

# Defining the sigmoid activation function
def sigmoid_appr(z):
    z = bfloat16(1.0)/bfloat16(2.0)*(z/(1+abs(z))+1)
    return z
...
hidden_layer_wt = bfloat16(np.random.rand(n_input, n_hidden) )
hidden_layer_bias =bfloat16(np.random.rand(n_hidden))

# Assigning initial random Weights and Biases at the output layer
output_layer_wt  = bfloat16(np.random.rand(n_hidden, n_output))
output_layer_bias = bfloat16(np.random.rand(n_output))
